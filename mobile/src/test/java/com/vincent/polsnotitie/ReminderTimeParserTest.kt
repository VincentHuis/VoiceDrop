...
class ReminderTimeParserTest {
    ...
    fun morgenOmTijd() {
        ...
        val r = ReminderTimeParser.parse("morgen om 9 uur tandarts", now())
    ...
    fun ochtendsForceertAm() {
        val r = ReminderTimeParser.parse("morgen om 9 uur 's ochtends tandarts", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun overEenUur() {
        val n = now()
        val r = ReminderTimeParser.parse("over een uur bellen", n)
    ...
    fun overEenKwartier() {
        val n = now()
        val r = ReminderTimeParser.parse("over een kwartier de was ophangen", n)
    ...
    fun overDrieKwartier() {
        val n = now()
        val r = ReminderTimeParser.parse("over drie kwartier bellen", n)
    ...
    fun overHalfUur() {
        val n = now()
        val r = ReminderTimeParser.parse("over een half uur eten", n)
    ...
    fun overTwintigMinuten() {
        val n = now()
        val r = ReminderTimeParser.parse("over 20 minuten oven uit", n)
    ...
    fun overTienMinuten() {
        val n = now()
        val r = ReminderTimeParser.parse("over tien minuten oven uitzetten", n)
    ...
    @Test
    fun weekdagMetTijd() {
        val r = ReminderTimeParser.parse("vrijdag om 15:30 vergadering", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun vanavondDefaultTijd() {
        val r = ReminderTimeParser.parse("vanavond Jan bellen", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun halfNegenOverdag() {
        ...
        val r = ReminderTimeParser.parse("half negen opstaan", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun losUurStandaardOverdag() {
        ...
        val r = ReminderTimeParser.parse("morgen om 1 uur naar de tandarts", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun avondsSchuiftNaarPm() {
        val r = ReminderTimeParser.parse("vanavond om 8 uur eten", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
        assertEquals(20, c.get(Calendar.HOUR_OF_DAY))
        assertEquals("eten", r.text)
    ...
    fun snachtsBlijftNacht() {
        val r = ReminderTimeParser.parse("om 1 uur 's nachts feest", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun expliciteKloktijdNietAangepast() {
        val r = ReminderTimeParser.parse("om 06:00 joggen", now())
        assertNotNull(r.remindAt)
        val c = cal(r.remindAt!!)
    ...
    fun geenTijdGeeftNull() {
        val r = ReminderTimeParser.parse("tandarts afspraak maken", now())
