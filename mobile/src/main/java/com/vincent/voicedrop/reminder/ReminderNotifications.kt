...
object ReminderNotifications {
    ...
    fun notifyReminder(context: Context, memoId: String, text: String) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setAutoCancel(true)
            .setContentIntent(pending)
            .addAction(snoozeAction(context, memoId, text, "10 min", 10))
            .addAction(snoozeAction(context, memoId, text, "1 uur", 60))
            .addAction(snoozeAction(context, memoId, text, "24 uur", 24 * 60))
            .build()
        notify(context, fireId(memoId), notification)
    }

    /** Snooze-actieknop: laat [ReminderReceiver] de herinnering [minutes] later opnieuw plannen. */
    private fun snoozeAction(
        context: Context, memoId: String, text: String, label: String, minutes: Int
        ...
        val pending = PendingIntent.getBroadcast(
            context, ("snooze$minutes$memoId").hashCode(), intent,
            PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
        )
        return NotificationCompat.Action.Builder(R.drawable.ic_reminder, label, pending).build()
    }

    /** Sluit de afgevuurde herinnering-notificatie (bv. na snoozen). */
    fun cancelReminder(context: Context, memoId: String) {
        NotificationManagerCompat.from(context).cancel(fireId(memoId))
    }
